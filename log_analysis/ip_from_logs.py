...
def most_freq_ips(logs: list[list[str]]) -> str:
    ...
    for log in logs:  # for each log
        ...
        freq[ip] = (
            freq.get(ip, 0) + 1
        )  # set dict key = ip, value is 0 if key doesn't exist or add 1 to existing value
    ...
    for ip in ip_list:
        ...
        for part in parts:  # for each octet
            result = (result << 8) + int(
                part
            )  # shift 8 bits to the left (octet) and add int value of each octet
    ...
    for i in range(n):  # for each list element
        for j in range(
            0, n - 1 - i
        ):  # for the unsorted portions (moves left as items get sorted)
            if (
